...
I split things into two GitHub Actions workflows. The first workflow handles the normal site deploy to the special `gh-pages` deployment branch on pushes to the `main` branch.

The second workflow runs on pull request events (opened, reopened, synchronize, and closed). For open PRs, it builds Jekyll with a PR-specific base URL like [https://petegraham.com/pr-preview/pr-13/](https://petegraham.com/pr-preview/pr-13/).
